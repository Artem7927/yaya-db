...
- GET /orders/:id/tasks — любая роль — разбор заказа в задачи готовки (orderToTasks: блюдо→техкарта→что брать из ПФ/ready/сырья + хватает ли; read-only)
- POST /orders/:id/item — MANAGER/SUPERVISOR/ASSEMBLER/KITCHEN — пометка БЛЮДА заказа {dishId, action:'take'|'done'}; списание по техкарте единожды на первой пометке done; закрытие последнего блюда → заказ сам 'done' (отдельной кнопки нет). Кухня НЕ может отменять/править/двигать статус после 'готов' (после 'done' endpoint отбивает)
- POST /orders/:id/status — MANAGER/SUPERVISOR/ASSEMBLER — cook/done/cancel, автосписание по техкарте (кухня больше НЕ использует — не может cancel)
...
- Жизненный цикл заказа: new→cook→done→fulfilled (или cancel). Статус ОДИН на всю систему (кухня пометила → админ видит тем же статусом). Списание по yaya_tech_v3 — единожды: на отметке блюда «готово» (POST /orders/:id/item action=done) через applyTechCard, флаг deducted=true ставится на закрытии последнего блюда; переход заказа в 'done' — АВТОМАТИЧЕСКИ при закрытии последнего блюда. Повторного списания на переходе в 'выдан' НЕТ (fulfill списания не делает). cancel возвращает остатки. Нехватка ПФ кафе (location='kitchen') — списывается что есть, остаток логируется как 'НЕДОСТАЧА ПФ в кафе' и возвращается в shortage/deduct_shortage (вариант B). items_status (JSONB на заказе) — статусы по блюдам {dishId:{qty,status:new|cook|done}}.
