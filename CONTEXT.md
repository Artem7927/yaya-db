...
- POST /transfer — MANAGER/WORKSHOP/KITCHEN — ПАЧКА передач ПФ, body {dir:'ws-ks', items:[{fromId,qty}]} (совместим со старым {fromId,qty}). ws-ks (двухфазный): списывает у цеха (pf_stock workshop) атомарно, создаёт пачку transfers status='pending' + transfer_items, performer='BUYER', кухне НЕ зачисляет (зачисление — только на /transfers/:id/accept). ks-ws — legacy мгновенный (списание+зачисление одним запросом, шапка status='accepted')
...
- Двухфазные передачи ПФ Цех→Кухня: цех списывает при POST /transfer, кухня зачисляет только на /transfers/:id/accept (reject возвращает объём цеху). «В пути» = пачка transfers status='pending' (+ строки transfer_items) — на складах НЕ отражается; ks-ws — legacy мгновенный (status='accepted').
